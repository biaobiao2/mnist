...
def extract_data(filename, num_images):
    """将图片数据转化成4维数组，并将像素值映射到[-0.5, 0.5]范围内
...
def main(_):
    ...
    #placeholder函数是在神经网络构建graph的时候在模型中的占位，此时并没有把要输入的数据传入模型，它只会分配必要的内存。
    ...
    # 在TensorFlow里，变量的定义和初始化是分开的，所有关于图变量的赋值和计算都要通过tf.Session的run来进行。
